...
export default function Home() {
    ...
    const procesarCola = async () => {
      ...
      for (const v of currentList) {
        if (v.estado === 'Descargando') {
          try {
            const res = await fetch(`http://127.0.0.1:8000/api/progreso/${v.id}`);
            if (res.ok) { 
      ...
      if (isActive) {
        ...
        descargasATrigger.forEach(v => {
          fetch('http://127.0.0.1:8000/api/descargar', {
            method: 'POST', 
  ...
  const abrirCarpetaReal = async () => {
    try {
      ...
      const res = await fetch('http://127.0.0.1:8000/api/abrir_carpeta', {
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
  ...
  const abrirExplorador = async () => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:8000/api/explorar');
      const data = await res.json();
  ...
  const togglePausa = async (id: string) => {
    ...
    if (video.estado === 'Descargando') {
      await fetch(`http://127.0.0.1:8000/api/cancelar/${id}`).catch(()=>{});
